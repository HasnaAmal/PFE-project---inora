...
// User: get all their conversations (open + closed)
...
router.post('/conversations/:id/messages', auth, async (req, res) => {
  try {
    ...
    const message = await prisma.message.create({
      data:    { conversationId: convoId, senderId: req.user.id, body },
      include: { sender: { select: { id: true, fullName: true, role: true } } }
    });
  ...
  try {
    const convos = await prisma.conversation.findMany({
      ...
      include: {
        ...
        claimedBy: { select: { id: true, fullName: true } }, // ✅ include claimer
        messages:  { orderBy: { createdAt: 'desc' }, take: 1 },
        _count: {
          select: {
            messages: {
...
router.patch('/admin/conversations/:id/unclaim', auth, async (req, res) => {
  try {
    ...
    req.app.get('io').to('admins').emit('convo_unclaimed', {
      convoId: updated.id
    });
...
router.patch('/admin/conversations/:id/close', auth, async (req, res) => {
  try {
    const convo = await prisma.conversation.update({
      where: { id: parseInt(req.params.id) },
      data:  { status: 'CLOSED', claimedById: null } // ✅ release claim on close
    });
    res.json(convo);
  } catch (err) {
    res.status(500).json({ message: err.message });
